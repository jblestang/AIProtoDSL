...
use aiprotodsl::walk::{
    message_extent, remove_message_in_place, validate_message_in_place,
    zero_padding_reserved_in_place, Endianness as WalkEndianness,
};
...
"#;
    ...
    for (val, valid) in [
        ...
        if valid {
            ...
            assert_eq!(
                decoded.get("code").and_then(Value::as_u64),
                Some(val as u64)
            );
        } else {
...
fn test_walk_message_extent_and_validate() {
    ...
    v.insert(
        "data".to_string(),
        Value::List(vec![Value::U8(0xaa), Value::U8(0xbb)]),
    );
    ...
    let encoded_bounded = codec_bounded
        .encode_message("Bounded", &vb)
        .expect("encode");
...
"#;
    ...
    zero_padding_reserved_in_place(&mut buf, 0, &resolved, endianness, "WithReserved")
        .expect("zero");
    assert_eq!(buf[1], 0);
    assert_eq!(buf[2], 0);
    assert_eq!(buf[0], 1);
...
fn test_presence_bits_encode_decode() {
    ...
    let decoded = codec
        .decode_message("WithPresence", &encoded)
        .expect("decode");
    ...
    let decoded2 = codec
        .decode_message("WithPresence", &encoded2)
        .expect("decode");
...
fn test_fspec_encode_decode() {
    ...
    let decoded = codec
        .decode_message("FspecRecord", &encoded)
        .expect("decode");
    assert_eq!(decoded.get("a"), Some(&Value::U8(10)));
    assert_eq!(decoded.get("b"), Some(&Value::U16(0x1234)));
}
...
fn test_asterix_family_parse() {
    ...
    assert!(
        resolved.get_enum("Cat034MessageType").is_some(),
        "Cat034MessageType enum should be resolved"
    );
    ...
    assert_eq!(
        ...
        Some("SectorCrossing"),
        "enum value 2 should be SectorCrossing"
    );
    ...
    let spare = track
        ...
        .expect("spare field");
    assert!(
        matches!(&spare.type_spec, TypeSpec::Padding(PaddingKind::Bits(4))),
        "TrackNumber.spare should be Padding(Bits(4)), got {:?}",
        spare.type_spec
    );
    ...
    assert_eq!(
        resolved.message_for_transport_values(&transport_values),
        Some("Cat048Record")
    );
    ...
    assert!(
        resolved.payload_repeated(),
        "ASTERIX payload is a list of records per data block (list<...> in selector)"
    );

    // Abstract data model: type definitions (ASN.1-like)
    assert!(
        resolved.get_type_def("DataSourceId").is_some(),
        "type DataSourceId"
    );
    assert!(
        resolved.get_type_def("Cat048Record").is_some(),
        "type Cat048Record"
    );
    assert!(
        resolved.get_type_def("Cat002Record").is_some(),
        "type Cat002Record"
    );
    ...
    assert!(
        td048.fields.len() > 10,
        "Cat048Record type should have many fields"
    );
    assert_eq!(td048.fields[0].name, "i048_010");
    assert!(
        td048.fields[0].optional,
        "i048_010 should be optional in abstract type"
    );
    assert!(
        matches!(td048.fields[0].abstract_type, AbstractType::TypeRef(ref s) if s == "DataSourceId")
    );
    ...
    let cat002_optionals: Vec<&str> = cat002
        .fields
        .iter()
    ...
    let bp002 = resolved
        .bitmap_presence_mapping_message("Cat002Record")
        .expect("Cat002Record has bitmap");
    ...
    let bp = resolved
        .bitmap_presence_mapping_message("Cat048Record")
        .expect("Cat048Record has bitmap");
    ...
    assert_eq!(
        child_040,
        Some("MeasuredPositionPolar"),
        "i048_040 is optional MeasuredPositionPolar"
    );
}
...
fn test_cat048_frame1_130_absent_decode() {
    ...
    let payload: Vec<u8> = vec![
        ...
        0x19, 0xc9, 0x35, 0x6d, 0x4d, 0xa0, 0xc5, 0xaf, 0xf1, 0xe0, 0x02, 0x00, 0x05,
        0x28, // 010..090 (14)
        0x3c, 0x66, 0x0c, 0x10, 0xc2, 0x36, 0xd4, 0x18, 0x01, 0x00, 0x07,
        0xb9, // 220(3),240(6),250(count=0),161(2) (12)
    ];
    ...
    match &result {
        Ok(values) => {
            ...
            assert!(
                list.is_empty(),
                "I048/130 should be absent for FSPEC 0xFD: {:?}",
                v130
            );
            // We must have decoded past 161
            assert!(
                values.get("i048_161").is_some(),
                "i048_161 should be present"
            );
        }
        Err(e) => panic!(
            "decode should succeed when mapping skips 130: {} (consumed={})",
            e, consumed
        ),
    }
}
...
fn test_dsl_lint_tabs_and_one_field_per_line() {
    ...
    assert!(
        errors.is_empty(),
        "compliant source should have no lint errors: {:?}",
        msgs
    );
...
fn test_nested_fspec_presence_stack_depth_5() {
    ...
    let v5 = HashMap::from([("v".to_string(), Value::List(vec![Value::U8(42)]))]);
    let v4 = HashMap::from([("e".to_string(), Value::List(vec![Value::Struct(v5)]))]);
    let v3 = HashMap::from([("d".to_string(), Value::List(vec![Value::Struct(v4)]))]);
    let v2 = HashMap::from([("c".to_string(), Value::List(vec![Value::Struct(v3)]))]);
    let v1 = HashMap::from([("b".to_string(), Value::List(vec![Value::Struct(v2)]))]);
    let msg = HashMap::from([("a".to_string(), Value::List(vec![Value::Struct(v1)]))]);

    let encoded = codec.encode_message("Nest5", &msg).expect("encode");
    // 1 (msg presence 0x80) + 1 (L1) + 1 (L2) + 1 (L3) + 1 (L4) + 1 (L5) + 1 (v) = 7 bytes
    assert!(
        encoded.len() >= 7,
        "encoded should have at least 7 bytes, got {}",
        encoded.len()
    );
    ...
    assert_eq!(
        leaf, 42,
        "leaf value should be 42 after decode (presence stack depth 5)"
    );
}
