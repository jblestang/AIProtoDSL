//! EGUI tool: load a PCAP + DSL and display decoded records in a tree view.
//! Build with: cargo run --bin decode_pcap_gui --features gui
...
struct GuiApp {
    ...
    resolved: Option<aiprotodsl::ResolvedProtocol>,
    selected_index: Option<usize>,
    load_error: Option<String>,
    _default_pcap: String,
    _default_dsl: String,
}
...
struct DecodedRecord {
    ...
    category: u8,
    message_name: String,
    values: std::collections::HashMap<String, aiprotodsl::Value>,
}

#[cfg(feature = "gui")]
impl GuiApp {
    fn new(cc: &eframe::CreationContext<'_>) -> Self {
        // Use a larger default font size across the app
...
#[cfg(feature = "gui")]
fn load_pcap_and_dsl(
    pcap_path: &str,
    dsl_path: &str,
) -> Result<(Vec<DecodedRecord>, aiprotodsl::ResolvedProtocol), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let protocol = parse(&src).map_err(|e| format!("DSL parse: {}", e))?;
    ...
    let mut file = std::fs::File::open(pcap_path)?;
    let mut probe = [0u8; 4];
...
#[cfg(feature = "gui")]
fn process_udp(
    codec: &aiprotodsl::Codec,
    resolved: &aiprotodsl::ResolvedProtocol,
    udp_payload: &[u8],
    packet_index: u64,
    ...
    while off + 3 <= udp_payload.len() {
        ...
        if let Ok(transport_values) = codec.decode_transport(block) {
            if let Some(msg_name) = resolved.message_for_transport_values(&transport_values) {
                if let Ok(res) = aiprotodsl::frame::decode_frame(codec, msg_name, block, Some(3)) {
        ...
        egui::CentralPanel::default().show(ctx, |ui: &mut egui::Ui| {
            if let (Some(idx), Some(resolved)) = (self.selected_index, self.resolved.as_ref()) {
                if let Some(record) = self.records.get(idx) {
                    ui.heading(format!("{} — packet {}, block offset {}", record.message_name, record.packet_index, record.block_offset));
                    ui.separator();
...
fn tree_ui(
    ...
    for k in keys {
        ...
        if let aiprotodsl::Value::List(lst) = v {
            if lst.is_empty() {
                continue;
            }
        }
...
fn value_tree_ui(
    ...
    use aiprotodsl::value_summary_line;
